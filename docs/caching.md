...
2. **Disk Cache:** A pre-compiled `.compiled.msgpack` file stored alongside the profile YAML files. Msgpack provides rapid binary deserialization, significantly outperforming YAML parsing.
...
flowchart TD
    ...
    CheckMem -- Yes, Hash Matches --> ReturnMem[Return Dict from RAM <br/>⏱️ 0.00ms]
    
    CheckMem -- No or Hash Mismatch --> CheckDisk{3. Is .compiled.msgpack on Disk?}
    CheckDisk -- Yes, Hash Matches --> ReadMsgPack[Read Msgpack]
    ReadMsgPack --> SaveMem[Save to Memory Cache]
    SaveMem --> ReturnDisk[Return Dict <br/>⏱️ 0.10ms]
    
    CheckDisk -- No or Hash Mismatch --> ParseYAML[4. Cache Miss! Parse YAML Files]
    ParseYAML --> CompileMsgPack[Compile to Msgpack & Save to Disk]
    CompileMsgPack --> SaveMem2[Save to Memory Cache]
    SaveMem2 --> ReturnParse[Return Dict <br/>⏱️ ~5.00ms]
```

## 🧠 Key Design Principles

### 1. Lazy Compilation (On-Demand)
The `.compiled.msgpack` file is **not** created when the administrator saves the YAML file or starts the Docker container. 
...
- If an administrator modifies `base.yaml` while the honeypot is running, the next SSH connection will result in a **hash mismatch**. The system will automatically discard the old caches, re-parse the YAML, and generate a fresh `.compiled.msgpack`.
...
└── .compiled.msgpack      # ⚡ Auto-generated binary cache
```

## 🧹 Maintenance

You do not need to manually delete `.compiled.msgpack` when editing profiles. The hashing system guarantees that your YAML changes will take effect on the very next connection. If you wish to forcefully clear the cache for debugging, you can simply delete the `.compiled.msgpack` files or restart the Python process.
