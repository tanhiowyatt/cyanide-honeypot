...
sys.path.append(str(Path(__file__).resolve().parent.parent.parent / "src")) # Adjusted path for new location
...
def load_hacker_commands(path):
    ...
    files.extend(glob.glob(str(Path(path) / "**" / "cowrie.json*"), recursive=True))
    files.extend(glob.glob(str(Path(path) / "cowrie.json*")))
    ...
    if state_file.exists() and not force:
        try:
            ...
            if time.time() - last_run < interval_days * 24 * 3600:
                print(f"[*] Skipping model training (less than {interval_days} days since last run). use --force to override.")
                return
        except Exception:
            pass
...
def build_kb():
    ...
    # Resolve path
    project_root = Path(__file__).resolve().parent.parent.parent
    if not Path(mitre_cve_path).is_absolute():
        mitre_cve_path = project_root / mitre_cve_path

    kb = KnowledgeBase()
    kb.load_data(mitre_cve_path)
    kb.build_index()
...
def main():
    ...
    if not (args.train_model or args.build_kb):
        print("Usage: --train-model and/or --build-kb")
        sys.exit(1)
