...
async def verify():
    await verify_profile(
        "configs/profiles/fs.ubuntu_22_04.yaml",
        "Ubuntu 22.04.3 LTS (Jammy Jellyfish)",
        "ubuntu",
        2023,
    )
    await verify_profile(
        "configs/profiles/fs.debian_11.yaml", "Debian GNU/Linux 11 (bullseye)", "debian", 2024
    )
    await verify_profile(
        "configs/profiles/fs.centos_7.yaml", "CentOS Linux 7 (Core)", "centos", 2024
    )
