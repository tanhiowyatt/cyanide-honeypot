...
def _stringify_paths(d):
    """Recursively convert Path objects to strings."""
    if isinstance(d, dict):
        return {k: _stringify_paths(v) for k, v in d.items()}
    if isinstance(d, list):
        return [_stringify_paths(v) for v in d]
...
def _resolve_env_path() -> Path:
    ...
    return env_path if env_path.exists() else Path(".env")
...
def _cast_val(val, cast, default, env_label: str):
    """Cast a config value to the desired type."""
    if cast is bool:
...
def _get_val(config_data, section, key, default=None, cast=str):
    ...
    if val is None and section in config_data and isinstance(config_data[section], dict):
        val = config_data[section].get(key)

    if val is None:
        return default

    return _cast_val(val, cast, default, primary_env)
...
def load_config(path: Any = None):
    ...
    config = _assemble_config_dict(config_data)

    try:
        model = CyanideConfig(**config)
        _CONFIG_EVENTS.append({"action": "config_schema_validated", "data": {"ok": True}})
        return _stringify_paths(model.model_dump())
    except ValidationError as e:
        _CONFIG_EVENTS.append({"action": "config_schema_error", "data": {"error": str(e)}})
        raise


def _assemble_config_dict(config_data: dict) -> dict:
    """Assemble the config dictionary from various sources."""
