...
class CyanideSFTPServer(asyncssh.SFTPServer):
    ...
    def opendir(self, path):
        ...
        self.open_files[handle] = {
            "node": node,
            "type": "dir",
            "iter": iter(node.children.values()),
        }
        return handle
    ...
    def readdir(self, handle):
        ...
        try:
            ...
            attrs = asyncssh.SFTPAttrs(
                size=node.size, uid=0, gid=0, permissions=mode, mtime=int(node.mtime.timestamp())
            )
    ...
    def open(self, path, pflags, attrs):
        ...
        mode = "r"
        if (
            (pflags & asyncssh.FXF_WRITE)
            or (pflags & asyncssh.FXF_CREAT)
            or (pflags & asyncssh.FXF_TRUNC)
        ):
            mode = "w"
            ...
            if obj["type"] == "file" and obj["mode"] == "w":
                ...
                if parent and isinstance(parent, Directory):
                    ...
                    if existing:
                        ...
                        existing.mtime = (
                            time.time()
                        )  # This fails type check if mtime is datetime, wait Node uses datetime.
                        import datetime
