...
class AnalyticsService:
    ...
    def _init_ml(self):
        try:
            ...
            config_path = self.config.get("ml", {}).get("model_path", "src/cyanide.ml/cyanideML.pkl")
            model_path = Path(config_path).parent
            
            if (model_path / "cyanideML.pkl").exists():
                self.logger.log_event("system", "system_status", {"message": f"Loading CyanideML pipeline from {model_path}..."})
                self.ml_pipeline = CyanideML(str(model_path))
                # Backward compatibility for server.py checks
                self.ml_filter = self.ml_pipeline
            else:
                self.logger.log_event("system", "system_warning", {"message": "ML models not found. Analysis will be skipped."})
    ...
    def analyze_command(self, cmd: str, username: str, src_ip: str, session_id: str, protocol: str):
        ...
        try:
            ...
            if is_anomaly:
                self.logger.log_event(session_id, "ml_anomaly", {
                    "score": result['anomaly_score'],
                    "error": result['reconstruction_error'],
                    "cmd": cmd,
                    "classification": result.get('classification'),
