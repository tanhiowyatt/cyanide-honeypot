...
class TelnetHandler:
    ...
    async def handle_connection(self, reader, writer):
        ...
        if not accepted:
            self.logger.log_event("system", "connection_rejected", {"src_ip": src_ip, "reason": reason})
            writer.close()
            return
        ...
        try:
            ...
            shell = ShellEmulator(fs, username, quarantine_callback=quarantine_hook, config=self.config)
            
            prompt = f"{username}@server:~$ "
            writer.write(prompt.encode())
            self.server._log_tty(tty_state, "OUT", prompt) # Keep using server's helper for now or move it?
