...
from cyanide.vfs.nodes import File, Directory
...
class CpCommand(FileOpCommand):
            ...
            if isinstance(src_node, Directory):
                ...
                if dest_is_dir:
                    ...
                    if dest_node:
                         ...
                         return "", f"cp: cannot overwrite non-directory '{dest_str}' with directory '{src_str}'\n", 1
                    ...
                    if not parent or not isinstance(parent, Directory):
                         return "", f"cp: cannot create directory '{dest_str}': No such file or directory\n", 1
    ...
    def _copy_node(self, src_node, parent_node, new_name):
        ...
        if isinstance(src_node, File):
            new_file = File(new_name, parent=parent_node, content=src_node.content, owner=self.username, group=self.username)
            parent_node.add_child(new_file)
        elif isinstance(src_node, Directory):
            new_dir = Directory(new_name, parent=parent_node, owner=self.username, group=self.username)
